...
- `2026-04-23` — Phase 6 landed (AI layer opens). `anthropic` gem + `config.x.ai` feature flag keyed on `ANTHROPIC_API_KEY` (bring-your-own-key; AI silently off when absent). `IssueTriageService` calls Claude via tool use (`suggest_triage` tool) so responses come back as a structured JSON with `priority`, `assignee_email`, `labels`, `rationale`; the system prompt is marked `cache_control: ephemeral` for prompt caching. `IssueTriageJob` (Solid Queue) runs out-of-band using `ActivityEvent.with_organization_guc(organization_id)` to set RLS context, writes suggestions to `issues.triage_suggestion` jsonb, and `broadcast_replace_to [issue, :triage]` so the issue page updates live. `IssueTriagesController` provides `PATCH/DELETE /projects/:slug/issues/:n/triage` for admin-gated apply/dismiss; apply sets priority + assignee from the suggestion and clears it. 62 tests / 151 assertions green.
